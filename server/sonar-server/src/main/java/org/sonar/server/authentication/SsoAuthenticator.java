...
public class SsoAuthenticator {
  ...
  public Optional<UserDto> authenticate(HttpServletRequest request, HttpServletResponse response) {
    ...
    if (login == null) {
      // We need to remove the JWT token in order for the user to not stay connected
      jwtHttpHandler.removeToken(request, response);
      return Optional.empty();
    }
    UserDto userDto = doAuthenticate(request, login);
    // FIXME Generation of token should be not done at each request !
    // But then the question : when should it be generated ? When do we know that a new token should be generated (Another user is authenticated on the same browser) ?
