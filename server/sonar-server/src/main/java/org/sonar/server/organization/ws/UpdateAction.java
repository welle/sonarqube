...
import static org.sonar.server.organization.ws.OrganizationsWsSupport.PARAM_UUID;
...
public class UpdateAction implements OrganizationsAction {
    ...
    WebService.NewAction action = context.createAction(ACTION)
      ...
      .setDescription(
        format("Update an organization.<br/>" +
          ...
          PARAM_UUID, PARAM_KEY))
      .setInternal(true)
      .setSince("6.2")
      .setHandler(this);

    action.createParam(PARAM_UUID)
      .setRequired(false)
      .setDescription("Organization uuid")
  ...
  private static void checkKeyOrUuid(@Nullable String uuid, @Nullable String key) {
    checkArgument(uuid != null ^ key != null, "Either '%s' or '%s' must be provided, not both", PARAM_UUID, PARAM_KEY);
  }
